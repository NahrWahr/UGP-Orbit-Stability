...
# ╔═╡ f08152ee-3d2a-45ed-8bdd-dccd66cd51c8
begin
	using StaticArrays
...
# ╔═╡ 44345c9d-eb2d-4be4-829d-264084ae8043
Ωalt(x)=begin
	x = x[1:3];
	μ = 1
	ϵ = 0.5
...
# ╔═╡ 3e0e57fc-f80e-4eac-8558-c6edc4b3af85
function H(x)
	# x[1:3] :: Position Vector
	# x[4:6] :: Momentum Vector
...
# ╔═╡ bf2fb9e0-2ce2-11ed-3b74-49a5c0210ba6
...
# ╔═╡ be9d44e8-40b7-4d92-b07c-3256b279b1c6
function dyneq(u, p, t)
	...
	dHtest(x) = ForwardDiff.gradient(Htest, x)

	return SVector{6}(J * dHtest(u));
end

# ╔═╡ 4822bf50-4680-40a3-8193-3679ebe0c4f0
begin
	xinit = SVector{6}([1, 0, 0, 0, 1, 1]/1.0)
	m = 1.0;
	G = 1.0;
	ϵ = 0.2;
	μ = 1
...
# ╔═╡ b3bd961f-09e4-4c76-a40b-67a60fcd7720
let ds = ContinuousDynamicalSystem(dyneq, xinit, [m, G, ϵ])

	diffparameters = (alg = RadauIIA5(), abstol = 1e-10, reltol = 1e-10, isoutofdomain=f)
		
	tr=trajectory(ds, 100.0, Δt=0.001;diffeq = diffparameters)[begin:100:end];
	println(size(tr))
	
	blim=2;
	Trajectory = plot(tr[:,1],tr[:,2],tr[:,3], xlim=(-blim,blim),ylim=(-blim,blim),zlim=(-blim,blim), title="Position Space")
	PotentialPlot = plot(map(x->Ωalt(x[1:3]), tr), title="Potential")
	KEPlot = plot(map(x->x[4:6]'x[4:6], tr), title="Kinetic Energy")
	Hamiltonian = plot(map(H, tr), title="Hamiltonian")

	x = range(0.1,1,10)
	g = (x,x,x,x,x,x)
	att = AttractorsViaRecurrences(ds, g; Δt = 0.1)
...
# ╔═╡ e204da43-c411-4f19-a602-f97c7d4138eb
let ds = ContinuousDynamicalSystem(dyneq, xinit, [m, G, ϵ])
	pint = poincaremap(ds, (6,0.0))
	dt = 500
	tr = Vector(undef, dt)
	...
	println("ω=" , ω);
	tr=hcat(Vector.(tr)...)
	poincare = plot(tr[1,:],tr[2,:],tr[3,:],title="Poincare")

	sint = stroboscopicmap(ds, ω;diffeq = (abstol = 1e-9, reltol = 1e-9, isoutofdomain=f))
...
# ╔═╡ c83e597d-b1eb-4747-a4ac-e2d15da5d8b2
function test(u, p, t)
	I = [i==j ? 1 : 0 for i=1:3,j=1:3];O = [0 for i=1:3,j=1:3]
	J = [O I; -I O]
...
# ╔═╡ 10c2efeb-9d27-463d-88b3-42f4ca1175f5
begin
	ds = ContinuousDynamicalSystem(dyneq, xinit, [m, G, ϵ])
	function chkem()
		rx() = (rand() + 0.5)/1.5
		ry() = (rand() - 0.5)/1
		x = [rx() ry() 0 0 1 1];
		return (gali(ds,500,3; u0 = x)[2][end], x[1:2]);
	end
	...
	begin
		...
		scatter(xi[1,:], xi[2,:], marker_z=xi[3,:])
	end
end

# ╔═╡ 45f5ac7a-12bf-4bfb-9125-4f44f10d3134
let xinit = SVector{6}(
	[0.958409789630418, 0.24156800128684597, 0, 0, 1, 1]/1.0); 

	diffparameters = (alg = Vern7(), abstol = 1e-9, reltol = 1e-9, isoutofdomain=f)
	
	ds = ContinuousDynamicalSystem(dyneq, xinit,[])
	tr=trajectory(ds, 44.0)#, Δt=0.001;diffeq = diffparameters)[begin:100:end];
	Trajectory = plot(tr[:,1],tr[:,2],tr[:,3])
end

# ╔═╡ 5bb70370-3f0e-4313-8e2f-60c3cbad8731
# ╠═╡ disabled = true
#=╠═╡
let ds = ContinuousDynamicalSystem(dyneq, xinit, [m, G, ϵ])
	# map(2:6) do i 
	...
	ti = tangent_integrator(ds)
	set_state!(ti, rand(6))
	#ChaosTools.gali(tinteg, tmax, Δt, threshold)
	gali(ds,35,3; u0 = rand(6))
end
  ╠═╡ =#

# ╔═╡ 4c1917d9-5bb0-4166-8df0-586b9331e257
# ╠═╡ disabled = true
#=╠═╡
let ds = ContinuousDynamicalSystem(dyneq, xinit, [m, G, ϵ])
	pro = [3,6]
	cstate = [0,0,0,0]
	pint = projected_integrator(ds, pro, cstate;diffeq = (abstol = 1e-9, reltol = 1e-9, isoutofdomain=f))
...
# ╔═╡ b9db5648-095f-4b21-9dd1-11f76380bd85
# ╠═╡ disabled = true
#=╠═╡
let p = ODEProblem(dyneq, xinit, (0.0,20.0))
	f(u,p,t) = begin
		x = u[1:3];
		R = √(+(@. x^2...))
		return R<0.3;
	...
	#tr = solve(p, McAte3(), dt=0.1)
	# plot(tr[1,:],tr[2,:],tr[3,:])
end
  ╠═╡ =#

# ╔═╡ 436bd521-62e7-4fdb-b171-aeee4dea8e80
# ╠═╡ disabled = true
#=╠═╡
let x=SVector(1.0,0.0)
	...
	function harm(u, p, t)
		...
		H(u) = KE(u) + Pot(u);
		dH(x) = ForwardDiff.gradient(H, x);
		return SVector{2}(reverse(dH(u)));
	end
	p = ODEProblem(harm, x, (0.0,20.0))
	tr = solve(p;alg = Rodas5(), abstol = 1e-9, reltol = 1e-9).u
	tr = hcat(Vector.(tr)...)
	plot(tr[1,:],tr[2,:])
	#tr = solve(p, McAte3(), dt=0.1)
end
...
# ╔═╡ 07bdb1b4-b267-4d8e-8c07-c545d19d944c
# ╠═╡ disabled = true
#=╠═╡
let x = [1,1,1], μ = 1
	R = √(+(@. x^2...));
...
git-tree-sha1 = "cdd00781a7fd16ca8aa7691b3f850d2403cacb87"
