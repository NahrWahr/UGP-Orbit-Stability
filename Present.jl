...
begin
	...
	diffparameters = (alg = Tsit5(), callback = cb)
end
...
let ds = ContinuousDynamicalSystem(dyneq, xinit, [])
	function chkem()
		...
		ry() = (rand() - 0.5)/2
		x = [rx() ry() 0 0 0 2];
		return (gali(ds,500,4; u0 = x, diffeq=diffparameters)[2][end], x[1:2]);
	end
...
let xinit = SVector{6}(
	[0.6242323817325509, -0.23311374763267517, 0, 0, 0, 2]/1.0); 
	
	ds = ContinuousDynamicalSystem(dyneq, xinit,[])
	tr=trajectory(ds, 100.0, Δt=0.001; diffeq = diffparameters)
	Trajectory = plot(tr[:,1],tr[:,2],tr[:,3])
end

# ╔═╡ 5db39d3a-4f38-4d5f-b40a-8de3e103ff7f
let xinit = SVector{6}(
	[0.34184044525093144, 0.15093876675042656, 0, 0, 0, 2]/1.0); 
...
# ╔═╡ 8698ca68-a221-4f8a-af1e-776305307b1d
# ╠═╡ disabled = true
#=╠═╡
let p = ODEProblem(dyneq, xinit, (0,100))
...
begin
	...
	H([0.9882277340763265, -0.3649561032994876, 0, 0, 0, 2])
end
  ╠═╡ =#
