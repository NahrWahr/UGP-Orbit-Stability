...
# ╔═╡ 24b8251b-50c0-4379-9ae6-b775450eeaa1
begin
	using StaticArrays
...
# ╔═╡ 292491bd-1f94-4711-93c0-d31ecd284a47
function H(u)
	x = u[1:3]
	p = u[4:6]
	KE = 0.5*p'p;
	...
	R = √(+(@. x^2...));
	θ = acos(x[3]/R);
	Pot += -μe/R + (2*μe*a^2)/(5*R^3)*ϵ*(3*(x[3]/R)^2 - 2)/2
...
# ╔═╡ 1660bdee-4f0d-46d1-bacb-3efbf9362623
...
# ╔═╡ e8457dea-4f9d-4cbe-8d8f-8cc3faff99ef
begin
	G = 6.673*1e-17
	M = 3.58e10
	a = .313/2
...
# ╔═╡ 8f87cab0-c04b-40ea-89c5-8a1e059dff9e
function norm(x)
	res = 0.0
	@inbounds for v in x
	  	res += v*v
...
# ╔═╡ bf4ee996-042f-4d51-9251-3669d1bf93d6
...
# ╔═╡ cafbf587-2aae-47f8-afa1-7420f0875a18
...
# ╔═╡ ac7b0f30-2104-4518-9d55-b4d16cb85b67
begin 
...
# ╔═╡ c2d0434e-1bd8-40f7-9c81-5ca2e0157248
begin
...
# ╔═╡ ea4fcfb4-0925-424c-bc84-c87cc1234517
begin
	N = 1000
...
# ╔═╡ 6efb3044-7ae7-47a8-9a10-db7943108622
function optimF(v)
...
# ╔═╡ 0dde55da-7a1b-457f-82fd-15290ac8e7f8
begin
	lower = [0.5, 0, 0, -1, -1, -1, 0.8*1e-2]
	upper = [1.5, 2π, 2π, 1, 1, 1, 1.2*3e-2]
	res = optimize(optimF, lower, upper, x₀, SAMIN(), Optim.Options(iterations=10^4, time_limit=20^1))
end

# ╔═╡ 21473b05-a2a9-4461-a7a4-328f55a00175
let x = SVector{6}(Spherical2Cartesian(res.minimizer));
	p = ODEProblem(dyneq, x, (0,10000))
	tr = solve(p, Vern7(), alg = Vern7(), reltol = 1e-13, abstol = 1e-13, callback = cb, saveat = 10)
	@show x
